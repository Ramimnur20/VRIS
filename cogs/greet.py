...
class Welcomer(commands.Cog):
    """
    Welcome message management cog for VOTOX.
    
    Supports both text and embed-based welcome messages with dynamic variable parsing.
    """
    
    def __init__(self, bot: commands.Bot):
        """
        Initialize the Welcomer cog.
    ...
    async def _get_guild_config(self, guild_id: int) -> Dict[str, Any]:
        """
        Retrieve or initialize guild welcome configuration from database.
        
        Args:
            guild_id: The ID of the guild.
        
        Returns:
            Dictionary containing the guild's welcome configuration.
        """
    ...
    def _create_error_embed(self, title: str, description: str) -> discord.Embed:
        ...
        embed.set_footer(text="VOTOX Welcome System")
        return embed
    ...
    async def greet_group(self, ctx: commands.Context):
        """
        Main command group for welcome message configuration.
        
        Displays a comprehensive help menu with all available subcommands.
        """
        embed = discord.Embed(
            title="🎉 VOTOX Welcome System - Help Menu",
            description="Configure welcome messages for your server using the commands below.",
            color=discord.Color.blurple()
        )
        
        embed.add_field(
            name=".greet channel [channel]",
            value="Set the channel where welcome messages will be sent.",
            inline=False
        )
        
        embed.add_field(
            name=".greet embed [on/off]",
            value="Toggle embed mode for welcome messages.",
            inline=False
        )
        
        embed.add_field(
            name=".greet message [text...]",
            value="Set the text welcome message (supports variables).",
            inline=False
        )
        
        embed.add_field(
            name=".greet test",
            value="Send a test welcome message to the configured channel.",
            inline=False
        )
        
        embed.add_field(
            name=".greet embed title [text...]",
            value="Set the embed title (only works if embed mode is ON).",
            inline=False
        )
        
        embed.add_field(
            name=".greet embed image [url]",
            value="Set the embed image URL (only works if embed mode is ON).",
            inline=False
        )
        
        embed.add_field(
            name=".greet embed footer [text...]",
        ...
        embed.set_footer(text="Use .greet <subcommand> for more information")
        
        await ctx.send(embed=embed)
    
    @greet_group.command(name="channel")
    @commands.has_permissions(administrator=True)
    async def greet_channel(self, ctx: commands.Context, channel: Optional[discord.TextChannel] = None):
        """
        ...
        if channel is None:
            if config["channel_id"] is None:
                embed = self._create_error_embed(
                    "No Channel Set",
                    "No welcome channel is currently configured. Please specify a channel."
                )
            else:
                ...
                embed = discord.Embed(
                    ...
                    description=f"Current welcome channel: {channel_mention}",
                    color=discord.Color.blurple()
                )
            await ctx.send(embed=embed)
            return
    ...
    @greet_group.command(name="mode")
    @commands.has_permissions(administrator=True)
    async def greet_mode(self, ctx: commands.Context, toggle: Optional[str] = None):
        """
        ...
        if toggle_lower == "on":
            ...
            embed = self._create_success_embed(
                "Embed Mode Disabled",
                "Welcome messages will now be sent as text messages."
            )
        else:
            embed = self._create_error_embed(
                "Invalid Option",
                "Use `.greet mode on` or `.greet mode off`"
            )
        
        await self._save_guild_config(ctx.guild.id, config)
        await ctx.send(embed=embed)
    
    @greet_group.command(name="message")
    @commands.has_permissions(administrator=True)
    async def greet_message(self, ctx: commands.Context, *, message: str = None):
        """
        Set the text welcome message.
        
        Args:
            ctx: The command context.
            message: The welcome message text (supports variables).
        """
        config = await self._get_guild_config(ctx.guild.id)
    ...
    async def greet_test(self, ctx: commands.Context):
        ...
        if config["channel_id"] is None:
            embed = self._create_error_embed(
                ...
                "Please set a welcome channel using `.greet channel [channel]`"
            )
            await ctx.send(embed=embed)
            return
        
        welcome_channel = ctx.guild.get_channel(config["channel_id"])
        ...
        if config["embed_enabled"]:
            ...
            if config["embed_image"]:
                welcome_embed.set_image(url=config["embed_image"])
        ...
        embed = self._create_success_embed(
            "Test Message Sent",
            f"Welcome message preview sent to {welcome_channel.mention}"
        )
        await ctx.send(embed=embed)
    
    @greet_group.group(name="embed")
    @commands.has_permissions(administrator=True)
    async def greet_embed_group(self, ctx: commands.Context):
        """
        Group for embed-specific configuration commands.
        
        Note: This allows .greet embed title, .greet embed image, etc.
        """
        if ctx.invoked_subcommand is None:
            await self.greet_mode(ctx)
    
    @greet_embed_group.command(name="title")
    @commands.has_permissions(administrator=True)
    async def greet_embed_title(self, ctx: commands.Context, *, title: str = None):
        """
        Set the embed title for welcome messages.
        ...
        if not config["embed_enabled"]:
            embed = self._create_error_embed(
                ...
                "Enable embed mode using `.greet mode on`"
            )
            await ctx.send(embed=embed)
            return
    ...
    @greet_embed_group.command(name="image")
    @commands.has_permissions(administrator=True)
    async def greet_embed_image(self, ctx: commands.Context, *, url: str = None):
        """
        Set the embed image URL for welcome messages.
    ...
    @greet_embed_group.command(name="footer")
    @commands.has_permissions(administrator=True)
    async def greet_embed_footer(self, ctx: commands.Context, *, footer: str = None):
        """
        Set the embed footer text for welcome messages.
        ...
        if not config["embed_enabled"]:
            embed = self._create_error_embed(
                ...
                "Enable embed mode using `.greet embed on`"
            )
            await ctx.send(embed=embed)
            return
    ...
    @greet_group.command(name="variables")
    async def greet_variables(self, ctx: commands.Context):
        """
        Display all available placeholder variables for welcome messages.
        """
        embed = discord.Embed(
            title="📋 Available Placeholder Variables",
            description="Use these placeholders in your welcome messages to display dynamic information.",
            color=discord.Color.blurple()
        )
        ...
        embed.add_field(
            name="<<guild.count>>",
            value="The server's total member count",
            inline=False
        )
        
        embed.set_footer(text="VOTOX Welcome System | Use these in .greet message and .greet embed title commands")
    ...
    async def on_member_join(self, member: discord.Member):
        """
        Handle the member join event and send the configured welcome message.
        
        Args:
            member: The member that just joined the guild.
        """
        ...
        welcome_channel = member.guild.get_channel(config["channel_id"])
        if welcome_channel is None:
            return
        ...
        try:
            ...
            print(f"Error sending welcome message for {member}: {e}")


# Setup function to load the cog
async def setup(bot: commands.Bot):
    """
    Load the Welcomer cog into the bot.
